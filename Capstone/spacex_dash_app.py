...
def get_pie_chart(entered_site):
    ...
    if entered_site == "ALL":
        ...
        filtered_df = filtered_df.loc[
            filtered_df["Launch Site"] == entered_site, ["class"]
        ].value_counts()
        fig = px.pie(
            filtered_df,
            values="count",
            names=["Failure", "Success"],
            title=f"Total Success Launches on {entered_site}",
        )
        return fig
...
def get_scatter_plot(entered_site, paylod_range):
    filtered_df = spacex_df[
        (spacex_df["Payload Mass (kg)"] >= paylod_range[0])
        & (spacex_df["Payload Mass (kg)"] <= paylod_range[1])
    ]
    if entered_site == "ALL":
        fig = px.scatter(
            filtered_df,
